...
def cot_wizard():
    cot = "\nGenerate the SQL after thinking step by step, and when you generate the SQL, you should use the following format: The final entire SQL code is: {YOUR_SQL}\n"
...
def few_shot_no_kg():
    ini_table = "CREATE TABLE singer\n(\n    singer_id         TEXT not null\n        primary key,\n    nation       TEXT  not null,\n    sname       TEXT null,\n    dname       TEXT null,\n    cname       TEXT null,\n    age    INTEGER         not null,\n    year  INTEGER          not null,\n    age  INTEGER          null,\n    salary  REAL          null,\n    city TEXT          null,\n    phone_number   INTEGER          null,\n--     tax   REAL      null,\n)"
    ini_prompt = "-- Using valid SQLite and understading External Knowledge, answer the following questions for the tables provided above.\n-- How many singers in USA who is older than 27?\nThe final SQL is: Let's think step by step."
...
def decouple_question_schema(datasets, db_root_path):
    ...
    return question_list, db_path_list, knowledge_list, data_list
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--mode', type=str, default='train')
    args_parser.add_argument('--output_path', type=str, default='/cpfs01/user/duhe/OpenRLHF/data/')
    args_parser.add_argument('--use_knowledge', type=str, default='False')
    args_parser.add_argument('--db_root_path', type=str, default='/cpfs01/shared/llm_code/hesiyang/data/bird/')
    ...
    eval_path = args.db_root_path + '/' + args.mode + '/' + args.mode +'.json'
    eval_data = json.load(open(eval_path, 'r'))
    # '''for debug'''
    # eval_data = eval_data[:3]
    # '''for debug'''
    save_path = args.output_path + 'bird_' + args.mode + '.jsonl'
    db_root_path = args.db_root_path + args.mode + '/' + args.mode + '_databases/'
    question_list, db_path_list, knowledge_list, data_list = decouple_question_schema(datasets=eval_data, db_root_path=db_root_path)
    assert len(question_list) == len(db_path_list) == len(knowledge_list)
    
    i = 0
    for db_path, question, knowledge, data in tqdm(zip(db_path_list, question_list, knowledge_list, data_list)):
        with open(save_path, 'a') as f:
            content = generate_combined_prompts_one(db_path=db_path, question=question)
            if i == 0:
                print(content)
            i += 1
