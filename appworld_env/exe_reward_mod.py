...
def collect_responses(labels, obs):
    ...
    for index, label in enumerate(labels):
        ...
        if obs[index]==None:
            ...
            print(f"{index}的obs是：{obs[index]}")
            response = agent.next_code_block(obs[index])
...
def environment_func(queries, responses, labels):
    ...
    for response,label in zip(responses,labels):
      ...
      obs_single=environment_func_single(response,label_json)
      obs.append(obs_single)

    return obs

    # pre_refs = [environment_func_single.remote(response,label) for response,label in zip(responses,labels)]
    ...
    #     else:
    #         pre_results.append("Error: Timeout")
...
def reward_func(queries, responses, labels):
  ...
  for label in labels:
    label_json = json.loads(label)
    test_tracker=evaluate_task(task_id=label_json['task_id'],experiment_name=label_json['experiment_name'])
